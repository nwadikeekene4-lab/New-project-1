...
export default function AdminProducts() {
  // ... [Keeping your existing state/functions: fetchProducts, addProduct, deleteProduct, updateProduct]
  ...
  return (
    <div className="konga-admin-wrapper" style={{ padding: '20px' }}>
      
      {/* ✅ BACK ARROW WITH INLINE CSS */}
      <Link 
        to="/admin/dashboard" 
        style={{
          display: 'inline-flex',
      ...
      <main className="konga-admin-main">
        {/* ... [Rest of your Product Add Form and Inventory Grid] */}
        <section className="konga-form-section">
          <form className="konga-quick-add-form" onSubmit={addProduct}>
            ...
            <div className="konga-form-row">
              ...
              <button type="submit" className="konga-add-submit-btn" style={{ backgroundColor: '#4caf50' }}>Add Product</button>
            </div>
          </form>
        </section>
              ...
              {editingId === p.id ? (
                <div className="konga-edit-overlay">
                  ...
                  <div className="konga-edit-actions">
                    <button className="konga-save-btn" onClick={() => updateProduct(p.id)} disabled={isSaving}>{isSaving ? "..." : "Save"}</button>
                ...
                <>
                  <div className="konga-img-container">
                    <img key={p.image} src={getImageUrl(p.image)} alt={p.name} style={{ opacity: p.syncing ? 0.5 : 1 }} />
                  ...
                  <div className="konga-product-actions">
                    <button className="konga-action-edit" onClick={() => { setEditingId(p.id); setEditName(p.name); setEditPrice(p.price); }}>Edit</button>
